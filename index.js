...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oiee sou o bot do sasuke @${num.split('@')[0]}\nSeja bem vindo(a) ao grupo fique sempre ativo prfvr🙂❤️ *${mdata.subject}*`
				...
				teks = `Foi de ralo hahaha @${num.split('@')[0]}👋😂`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛CALMAE LEK TO FAZENDO&⌛',
				success: '✔️PRONTO LEK✔️',
				error: {
					stick: '❌ Falha, ocorreu um erro adesivo ao converter a imagens ❌ ',
                    Iv: '❌ Link não é valido❌'
				},
				only: {
					group: '❌ Este comando só pode ser usado em grupos! ❌',
					ownerG: '❌ Este comando só pode ser usado pelo grupo proprietário! ❌',
					ownerB: '❌ Este comando só pode ser usado pelo S∆SUKE! ❌',
					admin: '❌ CALA A BOCA MEMBRO COMUM😂😂! ❌',
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ reply stickernya one ❌')
					reply(mess.wait)
